...
public class Tester {
	...
	public int countSpace(String str) {	
		...
		if(s[0].equals(" ")){
			return 1+ countSpace(str.substring(1,s.length));
	...
	public int div(int m, int n) throws IllegalArgumentException {
		if(n==0){ throw new IllegalArgumentException("bad number");}
	...
	// search all the elemntst between low and hiegh including low and hih to find the smallest object. 
		//swap the smallest object to the location at index low then increase by one then recursive call
	...
	private void recursiveSelectionSort(int a[], int low, int high) 
	{
		if(low >=high) return;
			...
			int minvalue=a[low],minindex=low;
		for(int search=low+1;search<=high;search++){
			if(a[search]<minvalue){
				minvalue=a[search];
				minindex=search;
			}
		}
